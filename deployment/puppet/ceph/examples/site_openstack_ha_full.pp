...
class role_ceph (
  ...
  class { 'ceph::conf':
    fsid            => $fsid,
    auth_type       => $auth_type,
    cluster_network => "${::network_br-mgmt}/24",
    public_network  => "${::network_br-ex}/24"
...
$floating_range  = '172.16.112.0/24'
...
#    class { 'ceph::radosgw': 
#	use_keystone => "true",
#	keystone_url => "${public_virtual_ip}:5000",
